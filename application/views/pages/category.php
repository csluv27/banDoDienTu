        ...
        <div class="features_items">
          ...
          <div id="featured-item-carousel" class="carousel slide product-grid" data-ride="carousel" data-interval="false">
            <div class="carousel-inner">
              <?php $chunks = array_chunk($allproductByCate_pagination, 12); ?>
              <?php foreach ($chunks as $key => $chunk) { ?>
                <div class="item <?php echo $key == 0 ? 'active' : '' ?>">
                  <div class="row">
                    <?php foreach ($chunk as $index => $cate) { ?>
                      <div class="col-sm-3 col-md-3 col-lg-3 product-item">
                        <div class="product-image-wrapper">
                          <form action="<?php echo base_url('add-to-cart') ?>" method="POST">
                            <div class="single-products <?php echo ($cate->quantity == 0) ? 'out-of-stock' : ''; ?>">
                              <div class="productinfo text-center">
                                <input type="hidden" value="<?php echo $cate->id ?>" name="product_id">
                                <input type="hidden" value="1" name="quantity">
                                <a href="<?php echo base_url('san-pham/'.$cate->id) ?>">
                                  <img src="<?php echo base_url('uploads/product/' . $cate->image) ?>" alt="<?php echo $cate->title ?>" />
                                </a>
                                ...
                                <div class="product-buttons">
                                  <a href="<?php echo base_url('san-pham/' . $cate->id) ?>" class="btn btn-default add-to-cart">
                                    <i class="fa fa-eye"></i>Thông tin
                                  </a>
                                  <button type="submit" class="btn btn-fefault cart" <?php echo ($cate->quantity == 0) ? 'disabled' : ''; ?>>
